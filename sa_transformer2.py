...
for i in range(100):
    ...
    # save preds
    preds_out_path = "{}/best_preds_test_{}.npy".format(opt.save_ckpt_dir, opt.data_dir.replace('/', '.'))
    np.save(preds_out_path, preds)


print('mean test c index', np.mean(preds))

print('sd test c index', np.std(preds))
