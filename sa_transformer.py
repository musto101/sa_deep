...
parser.add_argument("--return_counts", type=bool, default=True) # return_counts is True if the number of events and censored are returned
parser.add_argument("--mode", default='client') # mode is client if the model is run on a client, server if the model is run on a server
...
def clones(module, N): # return a ModuleList of N identical modules
    return nn.ModuleList([copy.deepcopy(module) for _ in range(N)]) # NOTE copy.deepcopy() is used to avoid sharing parameters between multiple layers of the same type in the Transformer model (https://pytorch.org/docs/stable/nn.html#torch.nn.ModuleList)
...
def attention(query, key, value, mask=None, dropout=None): # query, key, value are tensors of size (batch_size, max_time, d_model)
    ...
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k) # scores is a tensor of size (batch_size, max_time, max_time)
    if mask is not None:
...
class MultiHeadedAttention(nn.Module): # multi-headed attention module
    def __init__(self, h, d_model, dropout=0.1): # h is the number of heads, d_model is the dimension of the model
        ...
        self.linears = clones(nn.Linear(d_model, d_model), 4) # self.linears is a ModuleList of 4 identical Linear layers
    ...
    def forward(self, query, key, value, mask=None):
        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(1) # mask is a tensor of size (batch_size, 1, 1, max_time)
...
def evaluate(encoder, test_loader):
    ...
    with torch.no_grad():
        ...
        for features, durations, mask, label, is_observed_single in test_loader:
            is_observed.append(is_observed_single.cpu().numpy())
