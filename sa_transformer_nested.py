...
parser.add_argument('--max_time', type=int, default=50, help='Max number of months')
...
def evaluate(encoder, test_loader):
    ...
    with torch.no_grad():
        ...
        print('Test c index', test_cindex, 'mean abs error (OBS)', mae_obs)
...
#
#
# train_labels = pd.read_csv(opt.data_dir + '/train_labels.csv', header=0, index_col=False).transpose().to_numpy()
...
mc_score = []
def objective(trial):
    N = trial.suggest_int('N', 1, 10) # number of modules
    num_heads = trial.suggest_int('num_heads', 1, 10)
    d_model = trial.suggest_int('d_model', 1, 100)
    d_ff = trial.suggest_int('d_ff', 1, 10) # feed forward layer
    drop_prob = trial.suggest_float('drop_prob', 0.1, 0.5) # dropout
    lr = trial.suggest_float('lr', 1e-6, 1e-5)
    coeff = trial.suggest_float('coeff', 0.1, 3) # coefficient for the loss function
    coeff2 = trial.suggest_float('coeff2', 0.1, 3) # coefficient for the loss function
...
for i in range(2):
    ...
    # split train into train and validation
        ...
        # train_features = pd.read_csv(opt.data_dir + '/train_features.csv', header=0, index_col=False)
        # train_labels = pd.read_csv(opt.data_dir + '/train_labels.csv', header=0, index_col=False).transpose()
        ...
        opt.num_epochs = study.best_params['num_epochs']

        # save the best hyperparameters to csv
        best_params = pd.DataFrame(study.best_params, index=[0])
        best_params.to_csv('data/transformer_best_params.csv')
        ...
        # best_model = study.user_attrs["best_transformer"]

        score, test = main(features, labels, num_features)

        print('best c index val', score, 'test c index', test)
        preds.append(test)

    mean_score = np.mean(preds)
    mc_score.append(mean_score)
    # save mc_score to csv
    mc_score_df = pd.DataFrame(mc_score)
    mc_score_df.to_csv('data/transformer_mc_score.csv')


print('mean test c index', np.mean(mc_score))

print('sd test c index', np.std(mc_score))
